...
def admin_copy(p: dict) -> str:
    ...
    if len(merged) > 1:
        ...
        return (
            ...
            'Vì vậy trang này gom cả các truy vấn theo tên hiện hành và tên địa phương trước sắp xếp về một trang chính, '
...
def alias_section(p: dict) -> str:
    ...
    if not items:
        items.append("<p>Trang này dùng chính tên tỉnh/thành hiện hành làm trang chính cho nhóm tra cứu theo khu vực, không tạo thêm URL đồng nghĩa.</p>")
    return "".join(items)
...
<h2>Cách xử lý các truy vấn theo tên tỉnh/thành cũ</h2><p>Sau thay đổi đơn vị hành chính, người dùng vẫn có thể tìm bằng tên cũ. Hệ thống keyword của website giữ các truy vấn đó nhưng chỉ định <strong>một trang chính theo đơn vị hiện hành</strong> là <code>/khu-vuc/{escape(slug)}/</code>. Cách này giúp người đọc đến đúng ngữ cảnh hiện tại mà không tạo hàng loạt trang đổi tên địa phương giống nhau.</p>
