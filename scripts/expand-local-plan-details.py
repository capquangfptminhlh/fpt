...
<div class="local-plan-full-content"><div class="local-plan-panel"><h4>Quyền lợi & cấu hình đi kèm</h4><ul class="local-plan-benefit-list">{feature_html}</ul></div><div class="local-plan-panel"><h4>Gói này phù hợp với ai?</h4><p>{escape(fit)}</p><div class="local-plan-use-cases"><span>Gia đình</span><span>Nhà phố / căn hộ</span><span>Nhiều thiết bị</span><span>Kiểm tra hạ tầng trước</span></div></div></div>
