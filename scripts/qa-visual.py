...
def main() -> None:
    ...
    # Regression gate for the 320-430px quick-contact bar: never allow
    # desktop width or secondary copy to squeeze the three mobile actions.
    for token in (
    ...
    for page in pages:
        ...
        if contact_pos < 0:
            ...
            contact_tagged += 1
        if 'contact-dock.css?v=20260817-3' not in html:
            errors.append(f'{rel}: missing contact dock cache-busted v3 stylesheet')
        else:
            dock_v3 += 1
    ...
    print(
        ...
        f'apple_contact={contact_tagged}, mobile_dock_v3={dock_v3}, '
        f'external_fonts=0, responsive=ready'
    )
